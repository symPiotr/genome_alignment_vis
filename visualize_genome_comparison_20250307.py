...
if len(sys.argv) != 5:
	sys.exit('This script inputs two fasta files (reference and query), plus an output file generated by "process_gff_20250307.py".\n'
...
subprocess.Popen("tblastx -db_gencode 1 -query_gencode 1 -db %s -query %s -outfmt '6 qseqid qlen sseqid slen length pident qstart qend sstart send evalue' -evalue 10e-10 -out %s/blastn.txt" %  (Reference_fasta, Query_fasta, Work_dir), shell=True).wait()
...
VIS_DATA.close()
print("Output file for the visualization using Processing: ", Work_dir + Output_file)
print("All OK :)")
